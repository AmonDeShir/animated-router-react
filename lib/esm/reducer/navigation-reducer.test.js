...
describe('navigation reducer test', function () {
    ...
    it("shouldn't change state if action.type is incorrect", function () {
        var result = navigationReducer(initialState, {
            type: 'INCORRECT',
            payload: {},
        });
    ...
    it('should throw error if second route with the same name was added', function () {
        var state = navigationReducer(initialState, {
            type: 'ADD',
            payload: route,
        });
        expect(function () {
    ...
    it('should select route', function () {
        var result = navigationReducer(__assign(__assign({}, initialState), { routes: new Map([['/test', route]]) }), { type: 'SELECT', payload: '/test' });
    ...
    it('should select null if route to select not exist', function () {
        var result = navigationReducer(__assign(__assign({}, initialState), { routes: new Map() }), { type: 'SELECT', payload: '/notExist' });
        expect(result.selected).toBeNull();
        expect(result.selectPath).toBe('/notExist');
        expect(result.previousPath).toBe('/');
    });
    it('should select route with argument', function () {
        var result = navigationReducer(__assign(__assign({}, initialState), { routes: new Map([['/test', route]]) }), { type: 'SELECT', payload: '/test/2' });
        expect(result.selected).toEqual(route);
        expect(result.selectPath).toBe('/test');
        expect(result.previousPath).toBe('/');
    ...
    it("should set previous path to selected path", function () {
        var result = navigationReducer(__assign(__assign({}, initialState), { routes: new Map([['/test', route]]), selectPath: '/path' }), { type: 'SELECT', payload: '/test' });
    ...
    it("should set previous path to selected path with argument if selected path has argument", function () {
        var result = navigationReducer(__assign(__assign({}, initialState), { routes: new Map([['/test', route]]), selectPath: '/path', argument: '2' }), { type: 'SELECT', payload: '/test' });
        expect(result.previousPath).toBe('/path/2');
    });
    it("should remove path", function () {
        var route2 = __assign(__assign({}, route), { path: '/2' });
        var result = navigationReducer(__assign(__assign({}, initialState), { routes: new Map([
                ['/test', route],
                ['/2', route2],
